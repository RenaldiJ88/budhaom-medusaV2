...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 2. Extraer el ID del pago (MercadoPago puede enviar 'data.id' o 'id')
    const paymentId = body?.data?.id || body?.id;
    
    if (!paymentId) {
      console.error('❌ [WEBHOOK-MP] No se encontró payment_id en el body');
    ...
    if (!payment) {
      console.error('❌ [WEBHOOK-MP] No se pudo obtener información del pago:', paymentId);
      return NextResponse.json({ 
        received: true, 
    ...
    if (!cartId) {
      console.error('❌ [WEBHOOK-MP] No se encontró external_reference (cart_id) en el pago');
      return NextResponse.json({ 
        received: true, 
    ...
    console.log('🛒 [WEBHOOK-MP] Cart ID extraído del external_reference:', cartId);
    ...
    try {
      ...
      if (existingCart?.cart?.completed_at) {
        console.log('✅ [WEBHOOK-MP] Carrito ya completado, orden ya existe. Idempotencia aplicada.');
        console.log('📝 [WEBHOOK-MP] Tiempo de procesamiento:', Date.now() - startTime, 'ms');
        return NextResponse.json({ 
          received: true, 
      ...
      console.log('🔍 [WEBHOOK-MP] Verificando si existe orden para cart_id:', cartId);
      
    } catch (cartError: any) {
      // Si el carrito no existe, puede ser que ya se haya completado y eliminado
      console.log('⚠️ [WEBHOOK-MP] No se pudo recuperar el carrito (puede estar ya completado):', cartError.message);
    ...
    try {
      ...
      if (cartRes?.type === 'order' && cartRes?.order) {
        ...
        console.log('✅ [WEBHOOK-MP] Orden creada exitosamente desde webhook:', orderId);
        console.log('📝 [WEBHOOK-MP] Tiempo total de procesamiento:', Date.now() - startTime, 'ms');
        ...
        console.error('❌ [WEBHOOK-MP] La respuesta no contiene una orden válida:', cartRes);
        return NextResponse.json({ 
          received: true,
          processed: false,
      ...
      if (completeError.message?.includes('already completed') || 
        ...
        console.log('✅ [WEBHOOK-MP] Carrito ya completado (idempotencia):', completeError.message);
        return NextResponse.json({ 
          received: true,
          processed: false,
      ...
      console.error('❌ [WEBHOOK-MP] Error al completar carrito:', completeError);
