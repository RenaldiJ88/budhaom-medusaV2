...
class MercadoPagoProvider extends AbstractPaymentProvider<SessionData> {
  ...
  async initiatePayment(input: any): Promise<{ id: string, data: SessionData }> {
    ...
    try {
      ...
      // 3. Obtener Cart ID (Estrategia defensiva mejorada)
      // Intentamos múltiples estrategias para obtener el cart_id
      let cartId = input.context?.cart_id || 
                   input.cart_id || 
      ...
      if (!cartId && input.data?.session_id) {
        try {
          const paymentModuleService: IPaymentModuleService = this.container_.resolve(Modules.PAYMENT);
          const sessionId = input.data.session_id;
          
          this.logger_.info(`🔍 [MP-INFO] Buscando cart_id desde sesión: ${sessionId}`);
          
          // Intentamos obtener la sesión de pago para extraer el resource_id (cart_id)
          const paymentSession: any = await paymentModuleService.retrievePaymentSession(sessionId);
          
          // En Medusa 2.0, el resource_id puede estar en diferentes lugares
          const sessionResourceId = paymentSession?.resource_id || 
                                   paymentSession?.context?.resource_id ||
                                   (paymentSession as any)?.cart_id;
          
          if (sessionResourceId) {
            cartId = sessionResourceId;
            this.logger_.info(`✅ [MP-INFO] Cart ID obtenido desde sesión: ${cartId}`);
          }
        } catch (sessionError: any) {
          this.logger_.warn(`⚠️ [MP-WARN] No se pudo obtener cart_id desde sesión: ${sessionError.message}`);
        }
      }
      ...
      if (!cartId) {
        ...
        if (cartId) {
          this.logger_.warn(`⚠️ [MP-WARN] Usando session_id/idempotency_key como fallback: ${cartId}`);
        } else {
          this.logger_.error(`❌ [MP-ERROR] No se pudo obtener cart_id. Input keys: ${Object.keys(input).join(',')}`);
          throw new Error("No se pudo obtener el cart_id ni ningún identificador válido");
        }
      }
